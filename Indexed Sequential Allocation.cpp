...
int main()
{
	...
	for(i=0;i<n;i++)
	{
		...
		cout<<"\nEnter the index block (between 0 - "<<totalblk<<"): ";
		cin>>f[i].index;
		cout<<"\nEnter the blocks to be held by index block (between 0 - "<<totalblk<<"): ";
	...
	cout<<"\nAll files:\n";
	cout<<"\nFile No\t\tFile Name\tNo of Blocks\tIndex Block\tBlock Numbers\n";
	for(i=0;i<n;i++)
	{
		if(f[i].alloc==1)
		{
			cout<<i+1<<"\t\t"<<f[i].name<<"\t\t"<<f[i].size<<"\t\t"<<f[i].index<<"\t\t"<<f[i].blocks[0];
